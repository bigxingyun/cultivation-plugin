...
const HELP_CARD: Line[] = [
  T.title('修仙 · 指令'),
  T.sec('修炼'),
  T.list('　/闭关　　进入闭关（离线按时间戳累计修为）'),
  T.list('　/出关　　结束闭关后可执行写操作'),
...
const INTRO: Line[] = [
  ...
  T.blank(),
  T.head('一、核心循环'),
  T.prose('　闭关积累修为 → 经验池满 → 突破升境，为主轴。'),
  T.prose('　历练获取功法与丹药 → 提升属性与成功率 → 解锁更高档位任务。'),
  T.prose('　任务链推进叙事，节点定时推进，不消耗历练次数。'),
  T.blank(),
  T.head('二、离线结算'),
  T.prose('　闭关、历练队列、链节点均按时间戳惰性结算，无定时器依赖。'),
  T.prose('　离线期间收益照常累计，不设离线收益上限。'),
  T.prose('　典型日流程：出关 → 收取任务 / 排历练 / 突破 → 闭关。'),
  T.blank(),
  T.head('三、资源'),
  T.prose('　功法与丹药仅来自历练：打怪（灵材·稀有丹）、药田（丹药）、'),
  T.prose('　修习（功法）、复合（综合奖励）。'),
  T.prose('　灵材用于炼丹升品或功法升阶，为长期分配决策。'),
  T.blank(),
  T.head('四、进度参考'),
  T.prose('　纯闭关满级约 857 小时（约 36 天），计入突破失败约 58 天。'),
  T.prose('　功法与丹药满配可显著缩短周期。'),
  T.blank(),
  T.head('五、规则边界'),
  T.prose('　无血量与装备系统；增益仅来自功法与丹药。'),
  T.prose('　同时仅运转一门功法；突破失败退回该层进度，不损属性与功法。'),
]
    ...
    .action(shell(game, async (user, g, argv, args) => {
      ...
      if (key && key !== '指令') {
        const table: Record<string, Line[]> = {
          闭关: [T.list('/闭关　进入闭关，修为按时间戳累计。'), T.list('闭关期间仅允许查询与每日指令；写操作需先「出关」。')],
